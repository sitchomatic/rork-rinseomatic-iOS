...
class TestDebugViewModel {
    ...
    var retryableSessionCount: Int {
        sessions.filter { $0.status == .failed || $0.status == .connectionFailure || $0.status == .timeout }.count
    }
    ...
    func generateExportText() -> String {
        ...
        lines.append("Success: \(successCount) | Failed: \(failedCount) | Timeout: \(timeoutCount)")
    ...
    private func runWaves() async {
        ...
        for waveIndex in 0..<totalWaves {
            ...
            logger.log("TestDebug: Wave \(currentWave) complete — \(successCount) success, \(failedCount) failed, \(timeoutCount) timeout so far", category: .login, level: .info)
        }
    ...
    private func runSession(_ session: TestDebugSession, credential: TestDebugCredentialEntry, targetSite: LoginTargetSite, proxyTarget: ProxyRotationService.ProxyTarget, resolvedURL: URL? = nil) async {
        ...
        session.logs.append(PPSRLogEntry(
            ...
            level: session.status == .success ? .success : (session.status == .timeout ? .warning : .error)
        ))
    }
