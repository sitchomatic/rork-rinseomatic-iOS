...
class ConfidenceResultEngine {
    ...
    func evaluate(
        ...
        if tempScore >= disabledThreshold {
            ...
            outcome = .noAcc
            confidence = 0.3
            reasoning = "AMBIGUOUS — classified as No Account (success:\(String(format: "%.2f", successScore)) incorrect:\(String(format: "%.2f", incorrectScore)) disabled:\(String(format: "%.2f", disabledScore)))"
        }
