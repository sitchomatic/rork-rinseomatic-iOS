...
@MainActor
public final class AutomationPairSession {
    let sessionID = UUID()
    let task: PairedTask
...
public final class HeadlessWebViewWorker: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction) async -> WKNavigationActionPolicy {
        ...
        return domainAllowed ? .allow : .cancel
    }
    ...
    nonisolated public func userContentController(_ userContentController: WKUserContentController,
        ...
        Task { @MainActor in
            guard let body = rawBody as? [String: Any] else { return }
            ...
            await withTaskGroup(of: Bool.self) { group in
                for task in batch {
                    group.addTask {
                        ...
                        let session = await MainActor.run { AutomationPairSession(task: task, allowedDomains: allowedDomains) }
                        do {
