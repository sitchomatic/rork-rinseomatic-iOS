...
class CoordinateInteractionEngine {
    ...
    func coordinateClick(
        ...
        let jx = GaussianRandom.jitter(range: jitterPx)
        let jy = GaussianRandom.jitter(range: jitterPx)
    ...
    func checkNetworkIdle(executeJS: @escaping (String) async -> String?, timeoutMs: Int = 5000) async -> Bool {
        let instrumentJS = """
        (function(){
            if(!window.__netIdleTracking){
                window.__netIdlePending=0;
                function decPending(){
                    window.__netIdlePending=Math.max(0,(window.__netIdlePending||0)-1);
                }
                if(typeof window.fetch==='function'){
                    var origFetch=window.fetch;
                    window.fetch=function(){
                        window.__netIdlePending++;
                        try{
                            return origFetch.apply(this,arguments).finally(function(){decPending();});
                        }catch(e){
                            decPending();
                            throw e;
                        }
                ...
                XMLHttpRequest.prototype.send=function(){
                    if(this.__tracked){
                        window.__netIdlePending++;
                        this.addEventListener('loadend',function(){decPending();});
                    }
                    return origSend.apply(this,arguments);
                };
                window.__netIdleTracking=true;
            }
            return'INSTRUMENTED';
        })()
        """
        _ = await executeJS(instrumentJS)

        let checkJS = """
        (function(){
            if(document.readyState!=='complete')return'LOADING';
            if((window.__netIdlePending||0)>0)return'PENDING';
            return'IDLE';
        })()
        """
        let start = Date()
