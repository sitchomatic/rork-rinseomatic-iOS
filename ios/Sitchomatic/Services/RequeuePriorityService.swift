...
actor RequeuePriorityService {
    ...
    func prioritize(credentialId: String, username: String, outcome: LoginOutcome) async -> RequeueEntry? {
        let limit = await MainActor.run { AutomationSettingsPersistence.shared.load().maxRequeueCount }
        let count = (requeueCounts[credentialId] ?? 0) + 1
        requeueCounts[credentialId] = count

        if count > limit {
            DebugLogger.logBackground("RequeuePriority: \(username) exceeded max requeue count (\(limit)) — dropping", category: .automation, level: .warning)
            return nil
        }
        ...
        case .noAcc:
            ...
            if detCount >= maxDetectionBeforeDeprioritize {
                priority = .low
                reason = "no account (\(detCount)x detected — deprioritized)"
            } else {
                priority = .medium
                reason = "no account — proxy+URL rotation needed"
            }
            suggestProxy = true
        case .smsDetected:
            let detCount = (detectionCounts[credentialId] ?? 0) + 1
