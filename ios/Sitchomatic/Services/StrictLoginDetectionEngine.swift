...
class StrictLoginDetectionEngine {
    ...
    func evaluateImmediateOverrides(
        ...
        // Check for session cookie (session_id) as success indicator first (strong signal)
        if let jsExecutor = executeJS {
            let cookieCheckJS = """
            (function(){
                var cookies=document.cookie.split(';');
                for(var i=0;i<cookies.length;i++){
                    var c=cookies[i].trim();
                    if(c.indexOf('session_id=')===0||c.indexOf('sessionid=')===0||c.indexOf('auth_token=')===0||c.indexOf('user_session=')===0){
            ...
            """
            if let cookieResult = await jsExecutor(cookieCheckJS), cookieResult == "HAS_SESSION_COOKIE" {
                logger.log("StrictDetection P1: SUCCESS — session cookie detected", category: .evaluation, level: .success, sessionId: sessionId)
                return .success
            }
        }
    ...
    private func evaluatePostSubmitDOM(
        ...
        sessionId: String,
        label: String,
        settleMs: Int = AutomationSettingsPersistence.shared.load().pageStabilizationDelayMs
    ) async -> PartialResult? {
