...
class TrueDetectionService {
    ...
    nonisolated struct TrueDetectionConfig: Sendable {
        var hardPauseMs: Int = 50
        var tripleClickDelayMs: Int = 50
        var tripleClickCount: Int = 4
        var submitCycleCount: Int = 4
        var buttonRecoveryTimeoutMs: Int = 50
        var maxAttempts: Int = 4
        var postClickWaitMs: Int = 50
        ...
        static func fromSettings(_ settings: AutomationSettings) -> TrueDetectionConfig {
            var config = TrueDetectionConfig()
            config.hardPauseMs = settings.trueDetectionHardPauseMs
            config.tripleClickDelayMs = settings.trueDetectionTripleClickDelayMs
            config.tripleClickCount = settings.trueDetectionTripleClickCount
            config.submitCycleCount = settings.trueDetectionSubmitCycleCount
            config.buttonRecoveryTimeoutMs = settings.trueDetectionButtonRecoveryTimeoutMs
            config.maxAttempts = settings.trueDetectionMaxAttempts
            config.postClickWaitMs = settings.trueDetectionPostClickWaitMs
            config.postTypingDelayMs = settings.postTypingDelayMs
            config.interFieldDelayMs = settings.interFieldDelayMs
            config.preTypingDelayMs = settings.preTypingDelayMs
            config.errorRecoveryDelayMs = settings.errorRecoveryDelayMs
            config.postSubmitDelayMs = settings.postSubmitDelayMs
            config.cooldownMinutes = settings.trueDetectionCooldownMinutes
            config.emailSelector = settings.ignEmailSelector
            config.passwordSelector = settings.ignPasswordSelector
            config.submitSelector = settings.ignSubmitSelector
            if !settings.trueDetectionSuccessMarkers.isEmpty {
                config.successMarkers = settings.trueDetectionSuccessMarkers
            }
            if !settings.trueDetectionTerminalKeywords.isEmpty {
                config.terminalKeywords = settings.trueDetectionTerminalKeywords
            }
            if !settings.trueDetectionErrorBannerSelectors.isEmpty {
                config.errorBannerSelectors = settings.trueDetectionErrorBannerSelectors
            }
            return config
        }

        static func forSite(_ site: LoginTargetSite, settings: AutomationSettings) -> TrueDetectionConfig {
            var config = TrueDetectionConfig.fromSettings(settings)
            config.emailSelector = settings.emailSelector(for: site)
            config.passwordSelector = settings.passwordSelector(for: site)
            config.submitSelector = settings.submitSelector(for: site)
            return config
        }
    ...
    private func executeTrueDetectionStep(
        ...
        let domMs = Int(Date().timeIntervalSince(stepStart) * 1000)
        if !domReady {
            onLog?("TRUE DETECTION: DOM not ready after \(Int(domReadyTimeout))s (+\(domMs)ms)", .warning)
        } else {
            logger.log("TrueDetection: DOM ready +\(domMs)ms", category: .automation, level: .trace, sessionId: sessionId)
        }

        onLog?("TRUE DETECTION: Hard pause \(config.hardPauseMs)ms before interaction... (+\(domMs)ms)", .info)
        ...
        let emailResult = await fillHardcodedField(
            ...
            sessionId: sessionId,
            onLog: onLog,
            preTypingDelayMs: config.preTypingDelayMs
        )
        result.emailFilled = emailResult
        let emailMs = Int(Date().timeIntervalSince(t1) * 1000)
        if !emailResult {
            onLog?("TRUE DETECTION: Email fill FAILED on '\(config.emailSelector)' +\(emailMs)ms", .error)
            return result
        }
        onLog?("TRUE DETECTION: Email filled via '\(config.emailSelector)' +\(emailMs)ms", .success)
        logger.log("TrueDetection: inter-field delay \(config.interFieldDelayMs)ms", category: .automation, level: .trace, sessionId: sessionId)
        try? await Task.sleep(for: .milliseconds(config.interFieldDelayMs))
        ...
        let passMs = Int(Date().timeIntervalSince(t2) * 1000)
        if !passwordResult {
            onLog?("TRUE DETECTION: Password fill FAILED on '\(config.passwordSelector)' +\(passMs)ms", .error)
            return result
        }
        onLog?("TRUE DETECTION: Password filled via '\(config.passwordSelector)' +\(passMs)ms", .success)
        logger.log("TrueDetection: post-typing delay \(config.postTypingDelayMs)ms before submit", category: .automation, level: .trace, sessionId: sessionId)
        try? await Task.sleep(for: .milliseconds(config.postTypingDelayMs))

        logger.log("TrueDetection: submit selector='\(config.submitSelector)' cycles=\(config.submitCycleCount) clicks/cycle=\(config.tripleClickCount) delay=\(config.tripleClickDelayMs)ms", category: .automation, level: .info, sessionId: sessionId)
    ...
    private func fillHardcodedField(
        ...
        try? await Task.sleep(for: .milliseconds(preTypingDelayMs))
    ...
    private func checkTerminalErrors(
        ...
        // CRIMSON SWEEP: Legacy DOM-based red banner detection REMOVED.
        // Red banner detection is now handled exclusively by ThickRedDetectEngine (Phase 3 pixel sniper).
        // Only keyword-based terminal checks (temporarily disabled, has been disabled, SMS) remain above.

        return nil
    }

    // CRIMSON SWEEP: captureErrorBannerCrop REMOVED.
    // Legacy DOM-based banner cropping replaced by ThickRedDetectEngine pixel sniper.
    // Callers now use standard session.captureScreenshot() instead.
