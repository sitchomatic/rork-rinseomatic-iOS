...
class HumanTypingEngine {
    ...
    func typeCharByChar(text: String, executeJS: @escaping (String) async -> String?, sessionId: String, fieldName: String, minDelayMs: Int, maxDelayMs: Int) async -> Bool {
        ...
        for (index, char) in text.enumerated() {
            ...
            let delay = GaussianRandom.delay(minMs: minDelayMs, maxMs: maxDelayMs)
            if index > 0 && index % Int.random(in: 4...8) == 0 {
                let thinkPause = GaussianRandom.delay(minMs: 200, maxMs: 600)
    ...
    func typeSlowWithCorrections(text: String, executeJS: @escaping (String) async -> String?, sessionId: String, fieldName: String) async -> Bool {
        ...
        while i < chars.count {
            if Double.random(in: 0...1) < correctionChance && i > 2 {
                ...
                try? await Task.sleep(for: .milliseconds(GaussianRandom.delay(minMs: 300, maxMs: 800)))
                ...
                try? await Task.sleep(for: .milliseconds(GaussianRandom.delay(minMs: 200, maxMs: 500)))
            }
            ...
            let delay = GaussianRandom.delay(minMs: 120, maxMs: 350)
            if i > 0 && i % Int.random(in: 3...6) == 0 {
                try? await Task.sleep(for: .milliseconds(delay + GaussianRandom.delay(minMs: 300, maxMs: 900)))
            } else {
                try? await Task.sleep(for: .milliseconds(delay))
            }
