...
@MainActor
class PPSRAutomationEngine: PPSRCheckAutomationEngine {
    ...
    private func performCheck(session: LoginWebSession, check: PPSRCheck, sessionId: String = "") async -> CheckOutcome {
        ...
        if !appReady.ready && appReady.fieldsFound == 0 {
            ...
            if reloaded {
                ...
                if !retryReady.ready && retryReady.fieldsFound == 0 {
                    ...
                    await captureScreenshotForCheck(session: session, check: check, step: "no_fields", note: "No fields after reload", autoResult: .unknown)
                    return .connectionFailure
                }
            } else {
                failCheck(check, message: "FATAL: Page reload also failed")
                await captureScreenshotForCheck(session: session, check: check, step: "reload_failed", note: "Reload failed", autoResult: .unknown)
                return .connectionFailure
            }
        }
        ...
        case .uncertain: finalOutcomeStr = "noAcc"
