...
enum TimeoutResolver {
    // Always read live from persistence — never use a frozen default.
    private static var liveSettings: AutomationSettings {
        AutomationSettingsPersistence.shared.load().normalizedTimeouts()
    }
    ...
    static func resolveRequestTimeout(_ hardcoded: TimeInterval) -> TimeInterval {
        let pageLoad = liveSettings.pageLoadTimeout
        if pageLoad > 0 {
