                ...
                Color.clear
                    .overlay {
                        Image(uiImage: shot.displayImage)
                            ...
                            .withThumbnailAdjustment(for: shot.site, viewport: CGSize(width: 160, height: 120))
    ...
    private var singleSiteStats: some View {
        HStack(spacing: 6) {
            ...
            if album.unknownCount > 0 {
                miniStat(icon: "questionmark.circle.fill", count: album.unknownCount, color: .gray)
            }
            Spacer()
            ...
            Color.clear.frame(height: 180)
                .overlay {
                    Image(uiImage: screenshot.displayImage)
                        .resizable()
                        .aspectRatio(contentMode: .fill)
                        .withThumbnailAdjustment(for: screenshot.site, viewport: CGSize(width: 320, height: 180))
                        .allowsHitTesting(false)
                }
                .clipShape(.rect(cornerRadii: .init(topLeading: 12, topTrailing: 12)))
                .overlay(alignment: .topTrailing) {
...
struct LoginScreenshotCorrectionSheet: View {
    ...
    private var detectionReasoningSummary: String {
        ...
        if note.contains("lobby") || note.contains("dashboard") || note.contains("recommended") { reasons.append("Detected success indicators → Success") }
        if note.contains("green banner") { reasons.append("Green banner detected → Success") }
        if note.contains("no response") || note.contains("timeout") { reasons.append("No response / timeout → No Account") }
    ...
    private func submitAIFeedback() {
        let host = screenshot.isJoe ? "joefortunepokies.eu" : "ignitioncasino.eu"
