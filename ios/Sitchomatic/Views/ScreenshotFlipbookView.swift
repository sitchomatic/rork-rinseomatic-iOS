...
struct UnifiedScreenshotFlipbookView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                TabView(selection: $currentIndex) {
                    ForEach(Array(screenshots.enumerated()), id: \.element.id) { index, screenshot in
                        UnifiedFlipbookPage(screenshot: screenshot, isZoomed: $isZoomed)
                            .tag(index)
                    }
                }
                .tabViewStyle(.page(indexDisplayMode: .never))
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    if currentIndex < screenshots.count {
                        let screenshot = screenshots[currentIndex]
                        ShareLink(item: Image(uiImage: screenshot.fullImage), preview: SharePreview(screenshot.step.displayName, image: Image(uiImage: screenshot.fullImage))) {
                            Image(systemName: "square.and.arrow.up.circle.fill")
                                .font(.title2)
                                .symbolRenderingMode(.hierarchical)
                                .foregroundStyle(.white)
    ...
    private var unifiedFlipbookFooter: some View {
        VStack(spacing: 8) {
            if currentIndex < screenshots.count {
                let screenshot = screenshots[currentIndex]
                VStack(spacing: 6) {
                    HStack(spacing: 8) {
                        HStack(spacing: 4) {
                            ...
                            Text(screenshot.siteLabel.uppercased())
                    ...
                    HStack(spacing: 8) {
                        Label(screenshot.credentialEmail, systemImage: "person.fill")
                            .font(.system(.caption2, design: .monospaced))
                            .foregroundStyle(.white.opacity(0.7))
                            .lineLimit(1)
                        Spacer()
    ...
    private var unifiedScrubber: some View {
        ScrollViewReader { proxy in
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 4) {
                    ForEach(Array(screenshots.enumerated()), id: \.element.id) { index, screenshot in
...
struct UnifiedFlipbookPage: View {
